...
def trapezio(
    xi: float | int, xf: float | int, func: Callable[[int | float], float]
) -> float:
    h = xf - xi
...
def grau_3_aberta(
    ...
    h = (xf - xi) / 5
    return (5 * h / 24) * (
        11 * func(xi + h) + func(xi + 2 * h) + func(xi + 3 * h) + 11 * func(xi + 4 * h)
    )
...
def run_closed_newton_cotes_methods(
    a: float | int, b: float | int, func: Callable[[int | float], float]
) -> None:
    ...
    for method in get_closed_newton_cotes_methods():
        if method == trapezio:
            ...
            print(f"{Colors.BOLD}{Colors.PURPLE}\tRegra de grau 4:{Colors.END_FORMAT}")

        value, num_it = _calculate(a, b, func, method, caramelo=5)
        print(
            f"{Colors.BOLD}\t\tValor calculado: {value} \n\t\tNúmero de iterações: {num_it}{Colors.END_FORMAT}"
        )


def run_opened_newton_cotes_methods(
    a: float | int, b: float | int, func: Callable[[int | float], float]
) -> None:
    ...
    for method in get_opened_newton_cotes_methods():
        if method == trapezio_aberta:
            ...
            print(f"{Colors.BOLD}{Colors.PURPLE}\tRegra de grau 3:{Colors.END_FORMAT}")
        else:
            print(f"{Colors.BOLD}{Colors.PURPLE}\tRegra de grau 4:{Colors.END_FORMAT}")
