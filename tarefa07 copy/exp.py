from math import exp, tanh, cosh, sinh, pi
...
def gauss_legendre_3(xi: float | int, xf: float | int, func: Callable[[int | float], float]):
    ...
    func_xs_1 = f_bar_simp(xi, xf, xs_1, func)
    func_xs_2 = f_bar_simp(xi, xf, xs_2, func)
    func_xs_3 = f_bar_simp(xi, xf, xs_3, func)
    return ((xf - xi) / 2) * (func_xs_1 * w1 + func_xs_2 * w2 + func_xs_3 * w3)


def gauss_legendre_4(xi: float | int, xf: float | int, ci, cf, func: Callable[[int | float], float]):
    w1 = w4 = (18 + 30**0.5) / 36
    w2 = w3 = (18 - 30**0.5) / 36
...
def gauss_legendre_2(xi: float | int, xf: float | int, ci: float | int, cf: float | int, func: Callable[[float], float]):
    w1 = 1
    w2 = 1
